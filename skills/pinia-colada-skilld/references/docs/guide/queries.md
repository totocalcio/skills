...
export const useCurrentDeck = defineQuery(() => {
  ...
  const result = useQuery({
    ...
    query: () =>
      // you might need to cast the param or suffix it with `/api/decks?deckId=${route.params.deckId}`).then((res) => res.json()),
...
While using a `v-if="data"` or `datastate` property provides a more elegant and type-safe solution.
